...
int init_game_data() {
    ability_table = init_ability_table(test_memory_pool, &db_connection);
    potion_table = init_potion_table(test_memory_pool, &db_connection);
    gear_table = init_gear_table(test_memory_pool, &db_connection, ability_table);
    player = create_new_player(test_memory_pool);//initialize blank player
...
int free_game_data() {
    free_ability_table(test_memory_pool, ability_table);
    free_potion_table(test_memory_pool, potion_table);
    free_gear_table(test_memory_pool, gear_table);
    free_character(test_memory_pool, player);
    free_character(test_memory_pool, goblin);
    return 0;
}

int reset_goblin() {
    memory_pool_free(test_memory_pool, goblin);
    goblin = create_new_goblin(test_memory_pool);
    if (goblin == NULL) {
        return 1;
    }
