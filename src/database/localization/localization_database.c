#include "../database.h"
#include "localization_database.h"
 ...
 */
int attribute_key(char *attribute_name) {
    // Get the database connection
	DBConnection db_connection = *db_get_connection();
    //Check if the database is open
	if (!db_is_open()) {
		//log_msg(ERROR, "Database is not open", sqlite3_errmsg(db_connection.db));
	...
	if (rc != SQLITE_OK) {
	    //log_msg(ERROR, "Failed to prepare statement: %s", sqlite3_errmsg(db_connection.db));
	...
	if (rc != SQLITE_ROW) {
        //log_msg(ERROR, "Failed to execute statement: %s", sqlite3_errmsg(db_connection.db));
        fprintf(stderr, "Failed to execute statement: %s\n", sqlite3_errmsg(db_connection.db));
        sqlite3_finalize(statement);
        return -1;
	}
	// Get the attribute key
	int attribute_key = sqlite3_column_int(statement, 0);
